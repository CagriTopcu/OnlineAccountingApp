...
namespace OnlineAccountingAppServer.Application.Features.AppFeatures.AppUserFeatures.Commands.Login
{
    public sealed class LoginCommandValidator : AbstractValidator<LoginCommand>
    {
        ...
        {
            RuleFor(p => p.EmailOrUserName).NotNull().WithMessage("Mail ya da kullanıcı adı boş olamaz!");
            RuleFor(p => p.EmailOrUserName).NotEmpty().WithMessage("Mail ya da kullanıcı adı boş olamaz!");
